...
namespace EmailConsoleApp.Classes
{
    public class EmailService : IEmailService
    {
        ...
        public async Task<IEnumerable<ObservableMessage>> DownloadAllEmailsAsync()
        {
            try
            {
                ...
                List<ObservableMessage> result = new List<ObservableMessage>();

                for (int i = 0; i < inbox.Count; i++)
                {
                    var message = await inbox.GetMessageAsync(i);
                    UniqueId id = new UniqueId((uint)i);
                    result.Add(new ObservableMessage(message, id));
                }
        ...
        public async Task<IEnumerable<ObservableMessage>?> FetchAllMessages()
        {
            try
            {
                ...
                foreach (var summary in inbox.Fetch(0, inbox.Count, MessageSummaryItems.Envelope)) //what is max
                {
                    result.Add((ObservableMessage)summary);
                }
                return result;
            }
            catch (Exception ex)
            {
                throw new Exception($"Something went wrong while fetching mail inbox: {ex.Message}");
            }
        }
